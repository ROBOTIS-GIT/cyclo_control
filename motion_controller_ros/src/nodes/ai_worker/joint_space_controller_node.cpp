  ...
  dt_(0.01)
{
  ...
  right_traj_filtered_topic_ = this->declare_parameter("right_traj_filtered_topic",
      std::string("/leader/joint_trajectory_command_broadcaster_right/joint_trajectory_filtered"));
  left_traj_filtered_topic_ = this->declare_parameter("left_traj_filtered_topic",
      std::string("/leader/joint_trajectory_command_broadcaster_left/joint_trajectory_filtered"));
  ...
  try {
    ...
    qp_filter_ = std::make_shared<motion_controller::controllers::QPFilter>(kinematics_solver_,
        dt_);
    qp_filter_->setControllerParams(slack_penalty_, cbf_alpha_, collision_buffer_,
...
void JointSpaceController::rightTrajectoryCallback(
...
void JointSpaceController::leftTrajectoryCallback(
  const trajectory_msgs::msg::JointTrajectory::SharedPtr msg)
{
  if (!msg->points.empty()) {
...
void JointSpaceController::jointStateCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
  try {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Initial pose captured from %s (one-time seed for open-loop)",
