...
{
  m_settings = reinterpret_cast<OSQPSettings *>(c_malloc(sizeof(OSQPSettings)));
...
#else
  debugStream()
    << "[OsqpEigen::Settings::setAdaptiveRho] OSPQ has been set to EMBEDDED, "
    << "hence this setting is disabled."
    << std::endl;
...
#else
  debugStream()
    << "[OsqpEigen::Settings::setAdaptiveRhoInterval] OSPQ has been set to "
    << "EMBEDDED, hence this setting is disabled."
    << std::endl;
  unused(rhoInterval);
#endif
}
...
#else
  debugStream()
    << "[OsqpEigen::Settings::setAdaptiveRhoTolerance] OSPQ has been set to "
    << "EMBEDDED, hence this setting is disabled."
    << std::endl;
...
#else
  debugStream()
    << "[OsqpEigen::Settings::setAdaptiveRhoFraction] OSPQ has been set without "
    << "PROFILING, hence this setting is disabled."
    << std::endl;
  unused(adaptiveRhoFraction);
#endif  // ifdef PROFILING
#else  // # if EMBEDDED != 1
  debugStream()
    << "[OsqpEigen::Settings::setAdaptiveRhoFraction] OSPQ has been set to "
    << "EMBEDDED, hence this setting is disabled."
    << std::endl;
  unused(adaptiveRhoFraction);
#endif  // # if EMBEDDED != 1
...
void OsqpEigen::Settings::setPrimalInfeasibilityTollerance(
  const double primalInfeasibilityTolerance)
{
...
#else
  debugStream()
    << "[OsqpEigen::Settings::setDelta] OSPQ has been set to EMBEDDED, hence "
...
#else
  debugStream()
    << "[OsqpEigen::Settings::setPolish] OSPQ has been set to EMBEDDED, hence "
    << "this setting is disabled."
    << std::endl;
  unused(polish);
#endif
...
#else
  debugStream()
    << "[OsqpEigen::Settings::setPolishRefineIter] OSPQ has been set to "
    << "EMBEDDED, hence this setting is disabled."
    << std::endl;
...
#else
  debugStream()
    << "[OsqpEigen::Settings::setVerbosity] OSPQ has been set to EMBEDDED, "
    << "hence this setting is disabled."
    << std::endl;
  unused(isVerbose);
#endif
...
#else
  debugStream()
    << "[OsqpEigen::Settings::setCheckDualGap] OSQP version is lower than "
    << "v1.0.0, this setting is not available."
    << std::endl;
  unused(checkDualGap);
#endif
}
...
#else
  debugStream()
    << "[OsqpEigen::Settings::setTimeLimit] OSPQ has been set without "
    << "PROFILING, hence this setting is disabled."
    << std::endl;
  unused(timeLimit);
#endif
