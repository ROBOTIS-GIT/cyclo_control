...
{
  m_data = reinterpret_cast<OSQPData *>(c_malloc(sizeof(OSQPData)));
  m_data->P = nullptr;
  m_data->A = nullptr;
}
...
bool OsqpEigen::Data::isSet() const
{
  const bool areConstraintsOk = (m_data->m == 0) ||
    (m_isLinearConstraintsMatrixSet && m_isLowerBoundSet && m_isUpperBoundSet);
...
bool OsqpEigen::Data::setGradient(Eigen::Ref<Eigen::Matrix<c_float, Eigen::Dynamic, 1>> gradient)
{
  if (gradient.rows() != m_data->n) {
    debugStream()
      << "[OsqpEigen::Data::setGradient] The size of the gradient must be equal to the "
...
bool OsqpEigen::Data::setLowerBound(
  Eigen::Ref<Eigen::Matrix<c_float, Eigen::Dynamic, 1>> lowerBound)
{
  if (lowerBound.rows() != m_data->m) {
    debugStream()
      << "[OsqpEigen::Data::setLowerBound] The size of the lower bound must be equal to the "
      << "number of the variables."
      << std::endl;
    return false;
  }
...
bool OsqpEigen::Data::setUpperBound(
  Eigen::Ref<Eigen::Matrix<c_float, Eigen::Dynamic, 1>> upperBound)
{
  if (upperBound.rows() != m_data->m) {
    debugStream()
      << "[OsqpEigen::Data::setUpperBound] The size of the upper bound must be equal to the "
      << "number of the variables."
      << std::endl;
    return false;
  }
