...
# dependencies to build some parts are satisfied. For every dependency, it
# creates the following variables:
#
# OSQPEIGEN_USE_${Package}: Can be disabled by the user if he doesn't want to
# use that dependency. OSQPEIGEN_HAS_${Package}: Internal flag. It should be
# used to check if a part of OSQPEIGEN should be built. It is on if
# OSQPEIGEN_USE_${Package} is on and either the package was found or will be
# built. OSQPEIGEN_BUILD_${Package}: Internal flag. Used to check if OSQPEIGEN
# has to build an external package. OSQPEIGEN_BUILD_DEPS_${Package}: Internal
# flag. Used to check if dependencies required to build the package are
# available. OSQPEIGEN_HAS_SYSTEM_${Package}: Internal flag. Used to check if
# the package is available on the system. OSQPEIGEN_USE_SYSTEM_${Package}: This
# flag is shown only for packages in the extern folder that were also found on
# the system (TRUE by default). If this flag is enabled, the system installed
# library will be used instead of the version shipped within the framework.
...
macro(checkandset_optional_dependency package)
  ...
  set(${PREFIX}_USE_SYSTEM_${package}
      ${${PREFIX}_USE_${package}}
      CACHE
        INTERNAL
        "Use system-installed ${package}, rather than a private copy (recommended)"
        FORCE)
...
macro(OSQPEIGEN_DEPENDENT_OPTION _option _doc _default _deps _force)
  ...
  if(DEFINED _missing_deps)
    set(${_option}_disable_reason
        " (dependencies unsatisfied: \"${_missing_deps}\")")
    # Set a value that can be visualized on ccmake and on cmake-gui, but still
    # evaluates to false
    set(${_option}
        "OFF - Dependencies unsatisfied: '${_missing_deps}' - ${_option}-NOTFOUND"
        CACHE STRING "${_option_doc}" FORCE)
    ...
    # Set non-cache variable that will override the value in current scope For
    # parent scopes, the "-NOTFOUND ensures that the variable still evaluates to
    # false
    set(${_option} ${_force})
  endif()
