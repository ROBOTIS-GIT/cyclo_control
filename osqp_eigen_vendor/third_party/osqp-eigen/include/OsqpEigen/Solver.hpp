 ...
 * @date 2018
 */
#ifndef OSQPEIGEN__SOLVER_HPP_
#define OSQPEIGEN__SOLVER_HPP_
...
class Solver
{
  ...
  // Pointer to OSQPSolver struct.
  std::unique_ptr<OSQPSolver, std::function<void(OSQPSolver *)>> m_solver;
#else
  // Pointer to OSQPWorkspace struct.
  std::unique_ptr<OSQPWorkspace, std::function<void(OSQPWorkspace *)>> m_workspace;
#endif
  ...
  std::vector<Eigen::Triplet<c_float>> m_oldLinearConstraintsTriplet,
     ...
     */
  template<typename T>
  bool evaluateNewValues(
    const std::vector<Eigen::Triplet<T>> & oldMatrixTriplet,
...
public:
  ...
  bool setWarmStart(
    const Eigen::Matrix<T, n, 1> & primalVariable,
