// Copyright (C) 2023 Pierre Gergondet
//
...
#ifndef OSQPEIGEN__COMPAT_HPP_
#define OSQPEIGEN__COMPAT_HPP_
...
extern "C" {
extern OSQPCscMatrix * csc_spalloc(
  c_int m,
  c_int n,
  c_int nzmax,
  c_int values,
...
struct OSQPData
{
  ...
  OSQPCscMatrix * P;  ///< the upper triangular part of the quadratic objective
                      ///< matrix P (size n x n).
  ...
  OSQPInt nzmax)
{
  ...
  OSQPCscMatrix * M =
    static_cast<OSQPCscMatrix *>(calloc(1, sizeof(OSQPCscMatrix)));
  /* allocate the OSQPCscMatrix struct */
  if (!M) {
