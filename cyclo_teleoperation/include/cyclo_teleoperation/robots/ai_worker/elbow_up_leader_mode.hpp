...
private:
  void captureAnchor(ControlGroupMask groups);
  Eigen::Matrix<double, 6, 1> desiredVelocity(
    const Eigen::Affine3d & current,
    const Eigen::Affine3d & goal) const;
  Eigen::VectorXd elbowPreference(ControlGroupMask enabled_groups) const;

  ModeConfiguration configuration_;
  ControlGroupConfiguration left_group_;
  ControlGroupConfiguration right_group_;
  std::string left_elbow_link_;
  std::string right_elbow_link_;
